...
##    TopConstraint = property(get_TopConstraint, set_TopConstraint)
    ...
    def randomweightseriesunconstrained(self,
        ...
        #print 'rws len(self.SymbolsList)', len(self.SymbolsList)
        int_listlong = unconstrained_sample_posneg(longmin,longmax,len(self.SymbolsList),1)
        int_listshort = unconstrained_sample_posneg(shortmin,shortmax,len(self.SymbolsList),1)
        new_int_list = []
        i = 0
    ...
    def randomweightseriesunconstrained_old(self,
                                        maxpercent = 5
                                        ):
        #print 'rws len(self.SymbolsList)', len(self.SymbolsList)
        int_list = unconstrained_sample_posneg(0,maxpercent,len(self.SymbolsList),1)
        new_int_list = []
        i = 0
        for val in int_list:
            if self.SymbolsAndSignsList[i][1] == 'S':
    ...
    def portfolioriskreturnrandomweight(self,oftype='random',
        ...
        if oftype == 'equal':
            ...
            rws = self.randomweightseriesunconstrained(longmax,longmin,shortmax,shortmin)
