...
class perform:
    ...
    def annualizedreturns(self,):
        ...
        df__monthlyreturnsusingyahoosymbol = self.MonthlyReturnsDataframe
        df__monthlyreturnsusingyahoosymbol = df__monthlyreturnsusingyahoosymbol.dropna()
        df__monthlyreturnsusingyahoosymbol['e_pctchangeunitized'] = df__monthlyreturnsusingyahoosymbol['e_pctchange'] + 1
        #print df__monthlyreturnsusingyahoosymbol
        #print df__monthlyreturnsusingyahoosymbol #
        ls__monthlyreturnsusingyahoosymbol = df__monthlyreturnsusingyahoosymbol['e_pctchangeunitized'].values.tolist()
        #print ls__monthlyreturnsusingyahoosymbol
        #df__monthlyreturnsusingyahoosymbol
        listmultiplied = reduce(lambda x, y: x*y, ls__monthlyreturnsusingyahoosymbol)
        annualizedreturn = listmultiplied ** (float(1)/(float(len(ls__monthlyreturnsusingyahoosymbol)/float(12)))) - 1.0
        return annualizedreturn
        #print listmultiplied,len(ls__monthlyreturnsusingyahoosymbol)
        #print reduce(lambda x, y: x*y, [1,2,3,4,5,6])
    #    
    #    import operator
    #    import functools
    #    print functools.reduce(operator.mul, ls__monthlyreturnsusingyahoosymbol, 1)
        
    
        #return df__monthlyreturnsusingyahoosymbol
    def standarddeviationofmonthlyreturns(self,):
        df__monthlyreturnsusingyahoosymbol = df__monthlyreturnsusingyahoosymbol = self.MonthlyReturnsDataframe
        std_float = float(df__monthlyreturnsusingyahoosymbol['e_pctchange'].std())
        return std_float
