...
class perform:
    ...
    def set_StockHistoryDataframe(self,_StockHistoryDataframe):
        self._StockHistoryDataframe = _StockHistoryDataframe
    def get_StockHistoryDataframe(self):
        return self._StockHistoryDataframe
    _StockHistoryDataframe = property(get_StockHistoryDataframe, set_StockHistoryDataframe)
...
#    DailyReturnsDataframe = property(get_DailyReturnsDataframe, set_DailyReturnsDataframe)
    ...
    def __init__(self
        ...
        if period == 'monthly':
            self._StockHistoryDataframe = pp.stockhistorybackfilledtodatframeofstockhistoryinstancesusingcache(symbol,startdate_string,str(yesterday_date)) #,str(today_date))
            #self.MonthDayCharacter = 'M'
        else:
            self._StockHistoryDataframe = pp.stockhistorynobackfilltodataframeusingcache(symbol,startdate_string,str(yesterday_date)) #,str(today_date))
...
#        startdate_string = self.StartDateString
    ...
    def calculatereturns(self,period=''):
        # Parameters
        #startdate_string = '2004-12-31'
        #symbol = '^GSPC   ^OEX    ^VIX    ^OEX    ^MID   ^RUT
        symbol = self.Symbol 
...
#        print df_calculatereturns
        #ttttt
        #firstdate = df_calculatereturns.loc[0]['b_periodend']
        ls_indexes = df_calculatereturns.index.get_values()
        ...
        lastdate = ls_indexes[len(ls_indexes)-1] #df_calculatereturns.loc[len(df_calculatereturns)-1]['b_periodend']
        #print 'lastdate',lastdate
        #print df_calculatereturns
        df_calculatereturns['e_returnunitized'] = df_calculatereturns['e_'+logreturnorpctchange] + float(1.0)

        print ' --------------------- df_calculatereturns-----------------'
        print 'first date changed to:',str(df_calculatereturns.index.tolist()[0])
        firstdate = str(df_calculatereturns.index.tolist()[0])
        
        ls_calculatereturns = df_calculatereturns['e_returnunitized'].values.tolist()
        listmultiplied = reduce(lambda x, y: x*y, ls_calculatereturns)
        #print listmultiplied
...
#        df_calculatereturnsusingyahoosymbol = df_calculatereturnsusingyahoosymbol = self.MonthlyReturnsDataframe
...
if __name__=='__main__':
    ...
    o = perform(symbol=symbol,startdate_string=startdate,period='daily')
    #print '------ ReturnsDataframe ------'
    df = o.ReturnsDataframe
    #df = o.PriceHistoryDataframe
    print df
    print 'annualizedreturn log', o.annualizedreturn('logreturn')
    print 'annualizedreturn pctchange', o.annualizedreturn('pctchange')
